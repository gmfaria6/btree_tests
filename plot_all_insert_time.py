...
for element in [10, 100, 1000, 10000, 100000, 1000000, 10000000]:
    ...
    for test in range(1, 31):
        json_obj = json.load(open("{}_elements/btree_test_{}.txt".format(element, test)))
        avg += json_obj["total_insert_time"]
    y_btree.append(math.log((avg/30)*1000)/math.log(2))
...
for element in [10, 100, 1000, 10000, 100000, 1000000]: # , 10000000
    avg = 0
    for test in range(1, 31):
        json_obj = json.load(open("{}_elements/bplus_tree_test_{}.txt".format(element, test)))
        avg += json_obj["total_insert_time"]
    y_bplus_tree.append(math.log((avg/30)*1000)/math.log(2))
y_bplus_tree.append(20)
print(y_bplus_tree)
...
plt.ylabel("Tempo (log)", fontsize=14)
...
plt.savefig("all_elements_time.pdf", bbox_inches='tight')
